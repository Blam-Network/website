...
import type { Screenshot } from "@/src/api/odst/screenshots";
import { env } from "@/src/env";

export default function OdstScreenshotsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["odstScreenshots", page, pageSize, gamertag],
    queryFn: () =>
      api.odst.screenshots.query({
        page,
        pageSize,
  ...
  const updateURL = (newPage: number, newGamertag: string) => {
    ...
    const newUrl = `/odst/screenshots${queryString ? `?${queryString}` : ""}`;
          ...
          >
            {data.data.map((screenshot: Screenshot) => (
              <ScreenshotCard
                ...
                screenshotUrl={`${env.NEXT_PUBLIC_HALO3_API_BASE_URL}/halo3odst/screenshots/${screenshot.id}/view`}
