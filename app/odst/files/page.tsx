...
import type { FileshareFile } from "@/src/api/odst/fileshareFiles";
...
export default function OdstFilesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["odstFileshareFiles", page, pageSize, fileType],
    queryFn: () =>
      api.odst.fileshareFiles.query({
        page,
        pageSize,
  ...
  const updateURL = (newPage: number, newFileType: string) => {
    ...
    const newUrl = `/odst/files${queryString ? `?${queryString}` : ""}`;
                ...
                <CardContent sx={{ py: 1, px: 1.5, "&:last-child": { pb: 1 } }}>
                  <Stack spacing={0.5}>
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      <FileshareDownloadButton fileId={file.id} game="odst" />
                    </Box>
                    <Typography variant="caption" sx={{ color: "#B0B0B0" }}>
