...
import type { ServiceRecord } from "@/src/api/ares/serviceRecord";
...
export default function PlayersPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.ares.serviceRecords.query({
      page,
      pageSize,
  ...
  const updateURL = (newPage: number, newSearch: string) => {
    ...
    const newUrl = `/ares/players${queryString ? `?${queryString}` : ""}`;
          ...
          <Stack spacing={2}>
            {data.players.map((serviceRecord: ServiceRecord) => (
              <Link
                ...
                href={"/ares/player/" + serviceRecord.playerName}
                style={{
                  textDecoration: "none",
                  color: "unset",
                }}
