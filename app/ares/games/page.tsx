...
import type { RecentGame } from "@/src/api/ares/games";
...
export default function GamesPage() {
  ...
  const updateURL = (newPage: number, newGamertag: string) => {
    ...
    router.push(`/ares/games${queryString ? `?${queryString}` : ""}`);
  };
            ...
            <Table size="small">
              ...
              <TableBody>
                {data.data.map((game) => {
                  ...
                  const reportUrl = isCampaign ? `/ares/campaign-carnage-report/${gameId}` : `/ares/carnage-report/${gameId}`;
