...
import { FileShare } from "@/src/api/ares/fileShare";
import Link from "next/link";
import { format } from "date-fns";
import { PreviousGame } from "@/src/api/ares/previousGames";
...
export async function generateMetadata({ 
  ...
  if (searchParams?.viewScreenshot) {
    ...
    try {
      screenshot = await api.ares.screenshot.query({ id: searchParams.viewScreenshot });
    } catch {}
    ...
    if (screenshot) {
      ...
      const imageUrl = `${env.NEXT_PUBLIC_HALO3_API_BASE_URL}/ares/screenshots/${screenshot.id}/view`;
      const pageUrl = `${baseUrl}/ares/player/${encodeURIComponent(gamertag)}?viewScreenshot=${screenshot.id}`;
  ...
  try {
    serviceRecord = await api.ares.serviceRecord.query({ gamertag });
  } catch {}
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `${baseUrl}/ares/player/${encodeURIComponent(gamertag)}`,
      siteName: "Blam Network",
      images: [
        {
          url: emblemUrl,
...
export default async function Home({params}: {params: { gamertag: string }}) {
  ...
  const previousGamesResponse = await api.ares.playerPreviousGames.query({ gamertag, page: 1, pageSize: 45 });
        ...
        <Box sx={{ width: '100%', display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 4, alignItems: 'stretch' }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <Stack direction='row' justifyContent='space-between' alignItems='center' sx={{ mb: 2, pb: 1, borderBottom: '2px solid #7CB342' }}>
              ...
              <Link href={`/ares/games?gamertag=${encodeURIComponent(gamertag)}`} style={{ textDecoration: 'none' }}>
                <Typography 
                  variant='body2' 
                  sx={{ 
            ...
            {previousGames.length > 0 ? (
              <RecentGamesTable games={previousGames} stickyHeader={true} routeBase="/ares" />
            ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <Typography variant='body2' sx={{ fontSize: '0.75rem', color: '#B0B0B0', mb: 1 }}>
                          by{' '}
                          <Link href={"/ares/player/" + slot.header.author} style={{ color: '#4A90E2', textDecoration: 'none' }}>
                            {slot.header.author}
                          </Link>
                        </Typography>
                        {loggedIn && (
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 2, width: '100%' }}>
              {screenshots.map((screenshot: Screenshots[number]) => (
                <ScreenshotCard
                  ...
                  screenshotUrl={`${env.HALO3_API_BASE_URL}/ares/screenshots/${screenshot.id}/view`}
