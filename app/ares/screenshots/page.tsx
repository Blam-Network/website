...
import type { Screenshot } from "@/src/api/ares/screenshots";
...
export default function ScreenshotsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.ares.screenshots.query({
      page,
      pageSize,
  ...
  const updateURL = (newPage: number, newGamertag: string) => {
    ...
    const newUrl = `/ares/screenshots${queryString ? `?${queryString}` : ""}`;
            ...
            {data.data.map((screenshot: Screenshot) => (
              <ScreenshotCard
                ...
                screenshotId={screenshot.id}
                  screenshotUrl={`${env.NEXT_PUBLIC_HALO3_API_BASE_URL}/ares/screenshots/${screenshot.id}/view`}
