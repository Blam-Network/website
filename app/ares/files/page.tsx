...
import type { FileshareFile } from "@/src/api/ares/fileshareFiles";
...
export default function FilesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.ares.fileshareFiles.query({
      page,
      pageSize,
  ...
  const updateURL = (newPage: number, newFileType: string) => {
    ...
    const newUrl = `/ares/files${queryString ? `?${queryString}` : ""}`;
                  ...
                  <Stack spacing={0.5}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      {file.header.authorXuid && !isGuestXuid(file.header.authorXuid) ? (
                        <Link 
                          href={`/ares/player/${file.header.author}`} 
