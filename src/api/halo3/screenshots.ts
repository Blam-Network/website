...
export const playerScreenshots = publicProcedure.input(
    ...
    if (input.xuid) {
        ...
        url = `/halo3/players/${xuidParam}/screenshots?${params.toString()}`;
    } else {
        url = `/halo3/players/by-gamertag/${encodeURIComponent(input.gamertag!)}/screenshots?${params.toString()}`;
    }
...
export const screenshot = publicProcedure.input(
    ...
    const response = await halo3Axios.get(`/halo3/screenshots/${opts.input.id}`);
...
export const screenshots = publicProcedure.input(
    ...
    const response = await halo3Axios.get(`/halo3/screenshots?${params.toString()}`);
