...
export const datamineFilterOptions = protectedProcedure.query(async (opts) => {
    ...
    const response = await halo3Axios.get('/datamine/sessions/filter-options', {
        headers: {
...
export const datamineSessions = protectedProcedure.input(
    ...
    const response = await halo3Axios.get(`/datamine/sessions?${params.toString()}`, {
        headers: {
...
export const datamineSessionFilterOptions = protectedProcedure.input(
    ...
    const response = await halo3Axios.get(`/datamine/sessions/${input.sessionId}/filter-options`, {
        headers: {
            'Authorization': `Bearer ${ctx.jwtTokenString}`,
        }
    });
