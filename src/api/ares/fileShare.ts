...
export const fileShare = publicProcedure.input(
  ...
  const response = await aresAxios.get(`/ares/players/by-gamertag/${encodeURIComponent(input.gamertag)}/fileshare`);
  const parsed = FileShareSchema.safeParse(response.data);
  if (!parsed.success) throw new Error(`fileShare: schema mismatch. got=${JSON.stringify(response.data).slice(0, 500)}`);
  return parsed.data;
});
