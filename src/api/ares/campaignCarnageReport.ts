...
export const getCampaignCarnageReport = publicProcedure.input(
  ...
  const response = await aresAxios.get(`/ares/campaign-carnage-reports/${opts.input.id}`);
  const parsed = CampaignCarnageReportSchema.safeParse(response.data);
  if (!parsed.success) throw new Error(`campaignCarnageReport: schema mismatch. got=${JSON.stringify(response.data).slice(0, 500)}`);
  return parsed.data;
});
